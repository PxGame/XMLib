...
namespace XMLib
{
    ...
    public class TileCreatorEditor : EditorWindow
    {
        ...
        private const string DataFlag = "TileCreatorEditor_SaveData";

        private Camera _camera;
        private Vector2Int _endPos;
        private List<Vector2Int> _invaildPoss = new List<Vector2Int>();
        ...
        private void HelpView()
        {
            ...
            statusText += string.Format("选择点数:{0}\n选择周围点数:{1}\n无效点数:{2}\n", _selectPoss.Count, _selectAroundPoss.Count, _invaildPoss.Count);
        ...
        /// <returns></returns>
        private GameObject FindPrefab(string typeName, PosType posType, bool isHalf, ref float zAngle, out string objName)
        {
            ...
            //当前版本去除斜角
            posType &= (~(PosType.UpperLeft | PosType.LowerLeft | PosType.UpperRight | PosType.LowerRight));
            ...
            if (obj == null)
            {
                throw new Exception("未配置 " + objName + " 类型预制。");
            }
        ...
        /// <param name="zAngle"></param>
        /// <returns></returns>
        private GameObject InstObj(string typeName, string objName, GameObject preObj, Vector2Int gridPos, float zAngle)
        {
            ...
            obj.transform.rotation = Quaternion.Euler(0, 0, zAngle);
            ...
            if (!string.IsNullOrEmpty(tileRes.NewLayerName))
            {
                obj.layer = LayerMask.NameToLayer(tileRes.NewLayerName);
            }
        ...
        private GameObject InstPosObj(string typeName, Vector2Int pos, bool isHalf)
        {
            ...
            string objName = "";
            float zAngle = 0;
            GameObject preObj = FindPrefab(typeName, posType, isHalf, ref zAngle, out objName);
            return InstObj(typeName, objName, preObj, pos, zAngle);
        }
        ...
        private PosType CheckBorderPos(string typeName, Vector2Int pos)
        {
            ...
            {
                posType |= PosType.UpperLeft;
            }

            if (CheckPos(typeName, lowerLeft))
            {
                posType |= PosType.LowerLeft;
            }

            if (CheckPos(typeName, upperRight))
            {
                posType |= PosType.UpperRight;
            }

            if (CheckPos(typeName, lowerRight))
            {
                posType |= PosType.LowerRight;
            }
        ...
        private void DrawPosition()
        {
            ...
            length = _invaildPoss.Count;
            Handles.color = _data.invalidColor;
            for (int i = 0; i < length; i++)
            {
                worldPos = ToWorldPos(_invaildPoss[i]);
                Handles.DrawWireCube(worldPos, size);
            }
        ...
        /// <param name="typeName"></param>
        /// <param name="pos"></param>
        private void RepairHalfPos(string typeName, Vector2Int pos)
        {
            GameObject obj = InstPosObj(typeName, pos, false);
        ...
        private void RepairPos(string typeName, Vector2Int pos)
        {
            ...
            GameObject obj = InstPosObj(typeName, pos, true);
            SetTile(typeName, pos, obj);
        }
        ...
        private void UpdateSelectAroundPos(string typeName)
        {
            ...
            _invaildPoss = _invaildPoss.Except(_selectAroundPoss).ToList();
        }
        ...
        private void CreateChildRoot(string typeName)
        {
            ...
            if (!string.IsNullOrEmpty(tileRes.NewLayerName))
            {
                childRootObj.layer = LayerMask.NameToLayer(tileRes.NewLayerName);
            }
